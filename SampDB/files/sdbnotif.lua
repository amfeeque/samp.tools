...
local COLORS = {
	...
	[5] = {back = {0.41, 0.35, 0.80, 1},	text = {1, 1, 1, 1}, icon = {1, 1, 1, 1}, border = {1, 0, 0, 0}}
}
...
EXPORTS = {
    ...
    at = function(text, type, time, colors)
        table.insert(list, {
            text = text,
            type = type or 2,
            time = time or 4,
...
local newFrame = imgui.OnFrame(
    ...
    function(self)
        ...
        imgui.SetWindowPosVec2(imgui.ImVec2(resX - 10 - winSize.x, resY - winSize.y - 60))
        ...
        for k, data in ipairs(list) do
            ...
            local size = imgui.ImVec2(10 + iconSize.x + 10 + textSize.x + 10, 10 + textSize.y + 10)
...
function main()
    ...
    while true do
        ...
        for k, data in ipairs(list) do
            ...
            if os.clock() - data.start < 0.25 then
                list[k].alpha = bringFloatTo(0, 1, data.start, 0.25)
            elseif data.time - 4.0 < os.clock() - data.start then
                list[k].alpha = bringFloatTo(1, 0, data.start + data.time - 4.0, 4.0)
            end
