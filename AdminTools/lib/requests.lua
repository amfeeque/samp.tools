...
function _requests.make_request(request)
  ...
  local response = {}
  local ok
  local socket = string.find(full_request.url, '^http:') and not request.proxy and requests.http_socket or requests.https_socket
...
--Par through all the possible arguments for a request
...
function _requests.check_timeout(timeout)
  requests.http_socket.TIMEOUT = timeout or 15
