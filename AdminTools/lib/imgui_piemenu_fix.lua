...
local function BeginPiePopup(menuCtx, pName, iMouseButton)
	...
	if imgui.IsPopupOpen(pName) then
		...
		imgui.PushStyleVar(imgui.StyleVar.WindowRounding, 0.5)
		imgui.PushStyleVar(imgui.StyleVar.Alpha, 0.90)
		...
		if bOpened then
			...
			if menuCtx.m_iLastFrame < (iCurrentFrame - 1) then
				...
				sy = sy - 110
				menuCtx.m_oCenter = ImVec2(sx/2, sy/2)
			end
...
local function EndPiePopup(menuCtx)
	...
	for iIndex = 0, menuCtx.m_iMaxIndex do
		...
		local fMaxRadius = (fMinRadius + (fMenuHeight * oPieMenu.m_iCurrentIndex) / 4)
